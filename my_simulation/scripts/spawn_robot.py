...
from geometry_msgs.msg import Point, Pose, Quaternion, Twist, Wrench, PoseWithCovarianceStamped
...
class SpawnModel():
    ...
    def callSpawnService(self):
        ...
        # rospy.init_node('spawn_model') moved to main
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        db_world_name = rospy.get_param('world_name')
        db_path = rospkg.RosPack().get_path('my_simulation')
        conn = sql.connect(db_path + "/../test.db")
        conn.row_factory = sql.Row
        cursor = conn.cursor()
        cursor.execute("SELECT spawn_x, spawn_y, spawn_yaw FROM maps WHERE name=(?)", [db_world_name])
        db_world = cursor.fetchone()
        sm.initial_xyz[0] = db_world['spawn_x']
        sm.initial_xyz[1] = db_world['spawn_y']
        sm.initial_rpy[2] = db_world['spawn_yaw']

        sm.callSpawnService()

        init_pose_pub = rospy.Publisher("initialpose", PoseWithCovarianceStamped, queue_size=0)
        while init_pose_pub.get_num_connections() == 0:
            rospy.sleep(0.1)
        initialpose = PoseWithCovarianceStamped()
        initialpose.header.frame_id = 'map'
        initialpose.pose.pose.position.x = db_world['spawn_x']
        initialpose.pose.pose.position.y = db_world['spawn_y']
        quaternion = tft.quaternion_from_euler(0, 0, db_world['spawn_yaw'])
        _o = initialpose.pose.pose.orientation
        [_o.x, _o.y, _o.z, _o.w] = [quaternion[i] for i in range(0, 4)]
